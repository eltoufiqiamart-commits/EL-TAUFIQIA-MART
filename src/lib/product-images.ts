...
// ─── Admin/store image operations ───────────────────────────────────────────
// These operations intentionally do not use seller ownership. The store is
// managed by admins, while the customer-facing product catalog remains public.
export interface AdminProductContext { productId: string }

async function requireAdminProduct(productId: string): Promise<AdminProductContext> {
  const [product] = await db.select({ id: products.id }).from(products).where(eq(products.id, productId)).limit(1);
  if (!product) throw new OwnershipError("المنتج غير موجود");
  return { productId: product.id };
}

function buildAdminStoragePath(productId: string, extension: string) {
  return `products/admin/${productId}/${randomUUID()}.${extension}`;
}

export async function requireAdminProductContext(productId: string) {
  return requireAdminProduct(productId);
}

export async function uploadAdminProductImage(ctx: AdminProductContext, bytes: ArrayBuffer): Promise<StoredProductImage> {
  const processed = await validateAndProcessImage(bytes);
  const path = buildAdminStoragePath(ctx.productId, processed.extension);
  await ensureProductImagesBucket();
  const supabase = getSupabaseAdmin();
  const bucket = getProductImagesBucket();
  const { error: uploadError } = await supabase.storage.from(bucket).upload(path, processed.buffer, {
    contentType: processed.contentType, upsert: false, cacheControl: "31536000",
  });
  if (uploadError) throw new Error(`فشل رفع الصورة إلى المخزن: ${uploadError.message}`);
  const { data } = supabase.storage.from(bucket).getPublicUrl(path);
  const imageUrl = data.publicUrl;
  try {
    const existing = await db.select({ id: productImages.id }).from(productImages).where(eq(productImages.productId, ctx.productId));
    const isFirst = existing.length === 0;
    const [row] = await db.insert(productImages).values({
      productId: ctx.productId, imageUrl, storagePath: path,
      displayOrder: existing.length, isPrimary: isFirst,
    }).returning();
    if (isFirst) await syncMainImageUrl(ctx.productId);
    return { id: row.id, imageUrl: row.imageUrl, storagePath: path, isPrimary: row.isPrimary, displayOrder: row.displayOrder };
  } catch (err) {
    await supabase.storage.from(bucket).remove([path]).catch(() => {});
    throw err;
  }
...
export async function deleteAdminProductImage(ctx: AdminProductContext, imageId: string) {
  const [image] = await db.select().from(productImages).where(and(eq(productImages.id, imageId), eq(productImages.productId, ctx.productId))).limit(1);
  if (!image) throw new ImageValidationError("الصورة غير موجودة");
  const supabase = getSupabaseAdmin();
  const bucket = getProductImagesBucket();
  await db.transaction(async tx => {
    await tx.delete(productImages).where(eq(productImages.id, imageId));
    if (image.isPrimary) {
      const [next] = await tx.select().from(productImages).where(eq(productImages.productId, ctx.productId)).orderBy(productImages.displayOrder).limit(1);
      if (next) await tx.update(productImages).set({ isPrimary: true }).where(eq(productImages.id, next.id));
    }
    const [primary] = await tx.select({ imageUrl: productImages.imageUrl }).from(productImages).where(and(eq(productImages.productId, ctx.productId), eq(productImages.isPrimary, true))).limit(1);
    await tx.update(products).set({ mainImageUrl: primary?.imageUrl ?? null, updatedAt: new Date() }).where(eq(products.id, ctx.productId));
  });
  if (image.storagePath) await supabase.storage.from(bucket).remove([image.storagePath]).catch(() => {});
}

export async function setPrimaryAdminProductImage(ctx: AdminProductContext, imageId: string) {
  const [image] = await db.select({ id: productImages.id, imageUrl: productImages.imageUrl }).from(productImages).where(and(eq(productImages.id, imageId), eq(productImages.productId, ctx.productId))).limit(1);
  if (!image) throw new ImageValidationError("الصورة غير موجودة");
  await db.transaction(async tx => {
    await tx.update(productImages).set({ isPrimary: false }).where(eq(productImages.productId, ctx.productId));
    await tx.update(productImages).set({ isPrimary: true }).where(eq(productImages.id, imageId));
    await tx.update(products).set({ mainImageUrl: image.imageUrl, updatedAt: new Date() }).where(eq(products.id, ctx.productId));
  });
}

export async function reorderAdminProductImages(ctx: AdminProductContext, orderedImageIds: string[]) {
  const existing = await db.select({ id: productImages.id }).from(productImages).where(eq(productImages.productId, ctx.productId));
  const ids = new Set(existing.map(x => x.id));
  const requested = new Set(orderedImageIds);
  if (ids.size !== requested.size || [...ids].some(id => !requested.has(id))) throw new ImageValidationError("قائمة ترتيب الصور غير صحيحة");
  await db.transaction(async tx => {
    for (let i = 0; i < orderedImageIds.length; i++) await tx.update(productImages).set({ displayOrder: i }).where(eq(productImages.id, orderedImageIds[i]));
  });
}
