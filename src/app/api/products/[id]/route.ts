...
import { db } from "@/db";
import { products, productImages, categories, subcategories, brands, vehicleFitments, vehicleMakes, vehicleModels, reviews, profiles } from "@/db/schema";
...
export async function GET(
  ...
  try {
    ...
    const productReviews = await db
      .select({ id: reviews.id, rating: reviews.rating, comment: reviews.comment, createdAt: reviews.createdAt, customerName: profiles.fullName })
      .from(reviews)
      .leftJoin(profiles, eq(reviews.profileId, profiles.id))
      .where(and(eq(reviews.productId, id), eq(reviews.isApproved, true)))
      .orderBy(sql`${reviews.createdAt} DESC`);

    return NextResponse.json({ product, images, fitments, reviews: productReviews });
  } catch (error) {
