...
export default function CheckoutPage() {
  ...
  // Coupon validity and the authoritative discount are recalculated on the server.
  // The browser only previews the amount returned by the validation endpoint.
  const [couponPreview, setCouponPreview] = useState<{ code: string; discount: number; description?: string } | null>(null);
  const [couponLoading, setCouponLoading] = useState(false);
  const [couponError, setCouponError] = useState("");
  const couponDiscount = couponPreview?.discount ?? 0;
  const total = Math.max(0, subtotal - couponDiscount) + shippingFee - discount;
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            {/* Coupon */}
            <div className="bg-white rounded-xl border border-gray-100 p-5">
              <h3 className="font-bold text-gray-900 mb-3">كوبون الخصم</h3>
              <div className="flex flex-col sm:flex-row gap-2">
                <input
                  ...
                  className="flex-1 border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#1565C0]"
                />
                <button
                  type="button"
                  disabled={!form.couponCode.trim() || couponLoading}
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const data = await res.json();
                      if (res.ok) setCouponPreview({ code: data.coupon.code, discount: Number(data.coupon.discount), description: data.coupon.descriptionAr || undefined });
                      else setCouponError(data.error || "الكوبون غير صالح");
                    } catch {
                  ...
                  className="bg-[#1565C0] text-white px-5 py-2.5 rounded-lg text-sm font-bold disabled:opacity-50"
                >
              ...
              {couponPreview && (
                <div className="mt-3 p-3 rounded-lg bg-green-50 border border-green-100 text-sm text-green-700">
                  تم تطبيق الكوبون <span className="font-bold" dir="ltr">{couponPreview.code}</span> — خصم {formatPrice(couponPreview.discount)}
                  {couponPreview.description ? ` — ${couponPreview.description}` : ""}
                </div>
              )}
              {couponError && <p className="mt-2 text-xs text-red-600">{couponError}</p>}
              <p className="text-xs text-gray-400 mt-2">يتم التحقق من صلاحية الكوبون مرة أخرى عند تأكيد الطلب.</p>
            </div>
