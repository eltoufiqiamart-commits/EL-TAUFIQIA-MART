  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || "خطأ في تفعيل حساب البائع");
      }
    } catch {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow">
          ...
          <h2 className="text-xl font-black text-gray-900 mb-2">تم تفعيل حساب البائع بنجاح</h2>
          <p className="text-gray-500 text-sm mb-6">يمكنك الآن إدارة متجرك وإضافة منتجاتك من لوحة البائع.</p>
          <Link href="/seller/dashboard" className="inline-block bg-[#1565C0] text-white px-6 py-2.5 rounded-xl font-medium hover:bg-[#0D47A1]">الدخول إلى لوحة البائع</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="bg-white rounded-2xl border border-gray-100 shadow p-8">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">أكمل بياناتك وفعّل حسابك كبائع على توفيقية مارت</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={loading || !user} className="w-full bg-[#FF9900] hover:bg-[#E68900] text-white py-3 rounded-xl font-bold transition-colors disabled:opacity-60">
            {loading ? "جاري تفعيل حساب البائع..." : "تفعيل حساب البائع"}
          </button>
        </form>
      </div>
    </div>
