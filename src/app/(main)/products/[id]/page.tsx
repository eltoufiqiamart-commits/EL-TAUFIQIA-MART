...
interface Product {
  ...
  warranty?: string; mainImageUrl?: string;
...
export default function ProductPage() {
  ...
  const [reviews, setReviews] = useState<{id:string; rating:number; comment?:string|null; createdAt:string; customerName?:string|null}[]>([]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-5 mt-6">
        <div className="flex items-center justify-between mb-4"><h3 className="font-bold text-gray-900">تقييمات العملاء</h3><span className="text-sm text-gray-500">{reviews.length} تقييم</span></div>
        {reviews.length === 0 ? <p className="text-sm text-gray-500">لا توجد تقييمات معتمدة لهذا المنتج حتى الآن.</p> : <div className="space-y-4">{reviews.map(r => <div key={r.id} className="border-b last:border-0 border-gray-100 pb-4 last:pb-0"><div className="flex items-center gap-2"><span className="text-amber-500">{"★".repeat(r.rating)}<span className="text-gray-200">{"★".repeat(5-r.rating)}</span></span><span className="text-xs text-gray-500">{r.customerName || "عميل"}</span></div>{r.comment && <p className="text-sm text-gray-700 mt-2">{r.comment}</p>}</div>)}</div>}
      </div>
